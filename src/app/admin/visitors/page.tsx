    ...
    <div className="space-y-6">
      ...
      {!visits ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-zinc-400 text-sm">No visits recorded yet</p>
        </div>
      ) : (
              ...
              <tbody>
                {visits.map((v: any) => (
                  <tr key={v._id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {v.source ? (
                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-blue-500/10 text-xs text-blue-400">
                          <FiExternalLink className="w-3 h-3" />
                          {v.source}
                        </span>
                      ) : (
                        <span className="text-xs text-zinc-600">Direct</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      {v.country ? (
                    ...
                    <td className="px-4 py-3 text-right text-xs text-zinc-500">{formatDate(v.visitedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
